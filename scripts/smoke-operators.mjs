...
try{
 await mkdir(output,{recursive:true});temp=await mkdtemp(path.join(tmpdir(),'orchard-synthetic-operators-'));await writeFile(path.join(temp,'admin-token'),admin,{mode:0o600});
 ...
 const env={...process.env};for(const key of Object.keys(env))if(/^(TWILIO_|ORCHARD_)/.test(key))delete env[key];Object.assign(env,{TWILIO_ACCOUNT_SID:'',TWILIO_SECRET:'',TWILIO_SID:'',TWILIO_AUTH_TOKEN:'',TWILIO_FROM:'',RUST_LOG:'orchard_server=warn'});
 await assert.rejects(access(path.join(temp,'.env')),'The disposable working directory must not contain .env');
 child=spawn(path.join(root,'target/debug/orchard-server'),['--headless','--bind',`127.0.0.1:${port}`,'--data-dir',temp],{cwd:temp,env,stdio:['ignore','pipe','pipe']});let logs='';child.stdout.on('data',b=>logs+=b);child.stderr.on('data',b=>logs+=b);let spawnError;child.on('error',e=>spawnError=e);
