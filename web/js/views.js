...
export const Access = ({ vm, paging }) => html`<div class="stack"><${Panel} title="Security visibility"><p>Only security events explicitly reported by a collector appear here. This read-only view shows reported security events for the last hour. An empty list does not establish that no security activity occurred.</p><//><${Events} events=${vm.events.filter(e=>e.category==='security')} paging=${paging} empty="No security events reported. This does not establish that no security activity occurred."/></div>`;
